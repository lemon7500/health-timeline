...
def build() -> None:
    ...
    add_steps(
        doc,
        [
            "将“病程日历-1.0.1-release.apk”保存到手机。",
            "在“文件管理”中点击 APK。",
    ...
    for name, purpose in [
        ("日历", "查看、新增和修改病历；按病情分类筛选"),
        ("复查", "设置一次性或周期性复查，处理待复查事项"),
        ("用药", "登记药物、疗程和每日服药记录"),
        ("设置", "查看提醒权限，导出或恢复加密备份"),
    ]:
        cells = table.add_row().cells
    ...
    add_callout(doc, "密码无法找回", "备份包含病历、复查、用药记录和全部附件。密码不会保存在应用中，忘记后无法恢复该备份。", PALE_YELLOW)
    ...
    add_steps(
        ...
        [
            ...
            "打开“设置 → 从备份恢复”，选择备份文件。",
            "输入导出时设置的密码并确认。",
            "应用会先检查密码、版本、文件完整性和可用空间；全部通过后才替换当前数据。",
        ],
    )
    add_callout(doc, "恢复前先备份", "恢复会整体替换当前手机内的病程日历数据，请先为当前数据另做备份。验证失败时，现有数据不会被修改。", PALE_BLUE)

    add_heading(doc, "11. 更新应用", 1)
    add_body(doc, "如果手机已经安装旧版本，请直接点击新版 APK 进行覆盖安装。不要先卸载旧版，也不要清除应用数据。正常覆盖安装可保留已有记录；如果系统提示签名不一致，请停止操作并联系软件提供方。")
